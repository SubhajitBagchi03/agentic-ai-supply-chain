"""
Demand/Sales dataset upload endpoint.
POST /demand/upload
"""
...
async def upload_demand(file: UploadFile = File(...)):
    """
    Upload demand/sales CSV dataset.
    
    Expected columns: sku, date, quantity
    ...
    try:
        ...
        api_logger.info(
            f"Demand upload successful: {len(df)} rows",
            extra={"details": {"filename": file.filename, "rows": len(df)}}
        )

        return UploadResponse(
            status="success",
            message=f"Demand data loaded successfully: {len(df)} rows",
            rows_loaded=len(df),
            warnings=warnings,
            dataset_type="demand"
        )
