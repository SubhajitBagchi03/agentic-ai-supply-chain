...
import { uploadInventory, connectSheet } from '../services/api';
import {
...
export default function InventoryPage() {
  ...
  const handleConnect = async () => {
    ...
    setSheetResult(null);
    try {
      const res = await connectSheet(url, 'inventory');
        ...
        <div className="glass rounded-xl p-4 animate-slide-up">
          <div className="flex items-start gap-3">
            ...
            <div className="text-sm text-muted-foreground leading-relaxed">
              <strong className="text-foreground">How it works:</strong> Connect a Google Sheet for <strong>live auto-refresh every 30s</strong> — edit stock levels in the sheet and alerts appear automatically. Or upload a one-time CSV file.
            </div>
          </div>
        </div>

        {/* Combined Data Input Card */}
        <div className="glass rounded-xl p-6 space-y-5 animate-slide-up">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-semibold text-foreground">Inventory Data</h3>
              <p className="text-sm text-muted-foreground mt-0.5">
                Required: sku, name, warehouse, on_hand, safety_stock, lead_time_days, avg_daily_demand, supplier_id
              </p>
            </div>
            {isConnected && (
          ...
          <div>
            ...
            {uploadResult ? (
              <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4 animate-fade-in">
                ...
                <button onClick={() => { setUploadResult(null); setFile(null); }} className="text-xs text-muted-foreground hover:text-foreground mt-2 transition">
                  Upload another file
                </button>
              </div>
            ) : (
              <>
                <div
                  ...
                  onDrop={handleDrop}
                  onClick={() => document.getElementById('csv-inventory').click()}
                  className={`border-2 border-dashed rounded-2xl p-6 text-center transition-all cursor-pointer ${
                    dragActive ? 'border-blue-400 bg-blue-50/50' : 'border-black/8 hover:border-black/15'
                  }`}
                >
                  <input id="csv-inventory" type="file" accept=".csv" onChange={(e) => { setFile(e.target.files?.[0] || null); setUploadError(null); }} className="hidden" />
                  {file ? (
                    ...
                    <>
                      ...
                      <p className="text-sm text-muted-foreground">
                        Drag & drop CSV file or <span className="text-blue-600 font-medium">browse</span>
                      </p>
                    </>
                  )}
                </div>
                ...
                {file && (
                  ...
                  >
                    {uploading ? <><Loader2 className="w-4 h-4 animate-spin" /> Uploading...</> : <><Upload className="w-4 h-4" /> Upload Inventory CSV</>}
                  </button>
                )}
              </>
            )}
