...
import { uploadShipment, connectSheet } from '../services/api';
import {
...
export default function ShipmentsPage() {
  ...
  const handleConnect = async () => {
    ...
    setSheetResult(null);
    try {
      const res = await connectSheet(url, 'shipment');
  ...
  const handleUpload = async () => {
    ...
    setUploadError(null);
    try {
      const res = await uploadShipment(file);
      setUploadResult(res);
    } catch (err) {
      ...
      <div className="p-4 md:p-6 max-w-3xl mx-auto space-y-5">
        <div className="glass rounded-xl p-4 animate-slide-up">
          <div className="flex items-start gap-3">
            ...
            <div className="text-sm text-muted-foreground leading-relaxed">
              <strong className="text-foreground">How it works:</strong> Connect a Google Sheet for <strong>live auto-refresh every 30s</strong> — mark a shipment as "delayed" in the sheet and an alert appears on the dashboard automatically. Or upload a one-time CSV.
            </div>
          </div>
        </div>
        ...
        <div className="glass rounded-xl p-6 space-y-5 animate-slide-up">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-semibold text-foreground">Shipment Data</h3>
              <p className="text-sm text-muted-foreground mt-0.5">
                Required: shipment_id, sku, origin, destination, status, planned_date, carrier
              </p>
            </div>
            {isConnected && (
            ...
            {uploadResult ? (
              ...
              <>
                <div
                  ...
                  onClick={() => document.getElementById('csv-shipment').click()}
                  className={`border-2 border-dashed rounded-2xl p-6 text-center transition-all cursor-pointer ${dragActive ? 'border-violet-400 bg-violet-50/50' : 'border-black/8 hover:border-black/15'}`}>
                  <input id="csv-shipment" type="file" accept=".csv" onChange={(e) => { setFile(e.target.files?.[0] || null); setUploadError(null); }} className="hidden" />
                  {file ? (
                    <span className="flex items-center justify-center gap-2 text-sm text-foreground">
                      <FileText className="w-5 h-5 text-violet-600" /> {file.name} <span className="text-muted-foreground">({(file.size / 1024).toFixed(1)} KB)</span>
                    </span>
                  ) : (
                    <><Upload className="w-6 h-6 mx-auto mb-2 text-muted-foreground" /><p className="text-sm text-muted-foreground">Drag & drop CSV file or <span className="text-violet-600 font-medium">browse</span></p></>
                  )}
                </div>
                ...
                {file && (
                  <button onClick={handleUpload} disabled={uploading}
                    ...
                    {uploading ? <><Loader2 className="w-4 h-4 animate-spin" /> Uploading...</> : <><Upload className="w-4 h-4" /> Upload Shipment CSV</>}
                  </button>
                )}
              </>
            )}
